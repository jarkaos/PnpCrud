...
export default class PnpCrudWebPart extends BaseClientSideWebPart<IPnpCrudWebPartProps> {
        ...
        <div class="${ styles.container }">
          ...
          <div class="ms-Grid-row ${styles.row}">
            <div class="ms-Grid-col ms-u-lg10 ms-u-xl8 ms-u-xlPush2 ms-u-lgPush1">
              ...
              <ul id="items"></ul>
              <table id="content" class="table table-dark">
                <thead>
                  <tr>
                    <th>Title</th>
                    <th>Area</th>
  ...
  private setButtonsState(): void {
    ...
    for (let i:number = 0; i < buttons.length; i++) {
      ...
      if (listNotConfigured) {
        button.setAttribute('diseabled', 'diseabled');
      } else {
        button.removeAttribute('diseabled');
      }
    }
  }

  private setButtonsEventHandlers(): void {
    const webPart: PnpCrudWebPart = this;
  ...
  private readItems(): void {
    ...
    sp.web.lists.getByTitle(this.properties.listName)
      .items.select('Title', 'Id', 'Area', 'Telefon', 'Adress').get()
  ...
  private updateItemsHtml(items: IListItem[]): void {
    this.domElement.querySelector('#content tbody').innerHTML = items.map(item => `
      <tr>
        <td>${item.Title}</td>
        <td>${item.Area}</td>
        <td>${item.Telefon}</td>
